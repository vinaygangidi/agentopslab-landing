    ...
    <div style={{ fontFamily: "var(--font-sans)", background: 'var(--bg-base)', color: 'var(--text-primary)', minHeight: '100vh' }}>
      <style jsx>{`
        ...
        @media (max-width: 768px) {
          ...
          .photo-hero { width: 220px !important; height: 240px !important; }
        }
      ...
      <nav className="vg-nav">
        <div className="vg-nav-inner">
          ...
          <div className="nav-desktop vg-nav-links" style={{ display: 'flex' }}>
            ...
            <a href="mailto:vinay.gangidi@gmail.com" className="vg-btn-primary">
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '64px 28px 64px' }}>
        <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '64px', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
            ...
            <div style={{ textAlign: 'center', width: '100%' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '16px' }}>
                <a href="https://linkedin.com/in/vinaygangidi" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '7px 14px', background: '#EEF2FF', border: '1px solid var(--accent-border)', color: 'var(--accent-bright)', borderRadius: '8px', fontSize: '12px', fontWeight: '700', textDecoration: 'none' }}>
              ...
              <div className="stats-row" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '8px' }}>
                {[
                  ...
                  <div key={label} style={{ background: color + '12', border: '1px solid ' + color + '30', borderRadius: '10px', padding: '14px 10px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '8px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '.07em', lineHeight: '1.4' }}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--green)', background: 'var(--green-subtle)', border: '1px solid var(--green-border)', padding: '5px 12px', borderRadius: '100px', marginBottom: '20px' }}>
                <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--green)', display: 'inline-block' }} />
                AI Architect · Practitioner · Thought Leader
              </div>
              <h1 style={{ fontWeight: '800', fontSize: 'clamp(34px,4vw,52px)', lineHeight: '1.06', letterSpacing: '-.025em', marginBottom: '16px', color: 'var(--text-primary)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '12px', padding: '18px' }}>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '9px', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: '10px' }}>Current role</div>
                ...
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: 'var(--text-secondary)' }}>Oct 2020 - Present</div>
              </div>
              <div style={{ background: 'var(--bg-base)', border: '1px solid var(--border-default)', borderRadius: '12px', padding: '18px' }}>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '9px', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: '10px' }}>Available for</div>
      ...
      {/* THE JOURNEY */}
      <section style={{ background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)', padding: '80px 28px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '16px', lineHeight: '1.7', maxWidth: '60ch', marginBottom: '56px' }}>Each chapter taught me something the previous one could not. The technical depth came first. The strategic thinking came from applying it at scale and watching what broke.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
            {journey.map((chapter, idx) => (
              <div key={idx} style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '32px', paddingBottom: idx < journey.length - 1 ? '48px' : '0', borderBottom: idx < journey.length - 1 ? '1px dashed var(--border-subtle)' : 'none', marginBottom: idx < journey.length - 1 ? '48px' : '0' }}>
                ...
                <div>
                  <h3 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: '600', fontSize: '18px', color: 'var(--text-primary)', marginBottom: '12px', lineHeight: '1.4' }}>{chapter.title}</h3>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '15px', lineHeight: '1.8' }}>{chapter.body}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '80px 28px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '16px', lineHeight: '1.7', maxWidth: '60ch', marginBottom: '48px' }}>These are not opinions I picked up from reading. They came from shipping real systems, watching what failed, and figuring out why.</p>
        <div className="positions-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px', marginBottom: '32px' }}>
          {positions.map(({ icon: Icon, color, bg, title, body }) => (
            <div key={title} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '14px', padding: '28px', transition: 'box-shadow 0.2s' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: color + '18', border: '1px solid ' + color + '35', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
                <Icon size={20} color={color} />
              </div>
              <h3 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: '600', fontSize: '16px', color: 'var(--text-primary)', marginBottom: '12px', lineHeight: '1.4' }}>{title}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '14px', lineHeight: '1.75' }}>{body}</p>
            </div>
          ))}
        </div>
        <div style={{ background: 'var(--accent-subtle)', border: '1px solid var(--accent-border)', borderRadius: '14px', padding: '24px 28px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          <div>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: 'var(--accent-bright)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: '6px' }}>Published strategy</div>
            <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: '600', fontSize: '17px', color: 'var(--text-primary)' }}>Enterprise AI Strategy: Architecture Reference</div>
      ...
      <section style={{ background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)', padding: '80px 28px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '16px', lineHeight: '1.7', maxWidth: '60ch', marginBottom: '48px' }}>Not a list of every project. The ones that required the most thinking, shaped how I work, and produced the most durable outcomes.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {signatureWork.map((work, idx) => (
              <div key={idx} className="work-card" style={{ border: `1px solid ${openWork === idx ? work.color + '50' : 'var(--border-default)'}`, borderRadius: '14px', overflow: 'hidden', background: openWork === idx ? work.color + '0D' : 'var(--bg-surface)' }} onClick={() => setOpenWork(openWork === idx ? null : idx)}>
                <div style={{ padding: '24px 28px', display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                  <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: work.color + '18', border: `1px solid ${work.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontFamily: 'JetBrains Mono, monospace', fontWeight: '700', fontSize: '13px', color: work.color }}>
                    {String(idx + 1).padStart(2, '0')}
                  </div>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontWeight: '600', fontSize: '16px', color: 'var(--text-primary)', lineHeight: '1.4', marginBottom: '6px' }}>{work.title}</h3>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '3px 10px', background: work.color + '15', border: `1px solid ${work.color}35`, borderRadius: '100px', fontSize: '12px', color: work.color, fontWeight: '500' }}>
                      <Award size={10} /> {work.outcome}
                    </div>
                  </div>
                  <ChevronRight size={16} color={openWork === idx ? work.color : 'var(--text-muted)'} style={{ flexShrink: 0, marginTop: '4px', transform: openWork === idx ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }} />
                </div>
                {openWork === idx && (
                  <div style={{ padding: '0 28px 28px', borderTop: `1px solid ${work.color}30` }}>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '14px', lineHeight: '1.8', marginTop: '20px' }}>{work.what}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '80px 28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '12px', marginBottom: '32px' }}>
          {[
            ...
            <div key={name} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '10px', padding: '16px 20px', display: 'flex', gap: '12px', alignItems: 'center' }}>
              <div style={{ width: '34px', height: '34px', borderRadius: '9px', background: color + '18', border: '1px solid ' + color + '35', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Award size={16} color={color} />
              </div>
              <div>
                <div style={{ fontWeight: '600', fontSize: '13px', color: 'var(--text-primary)', lineHeight: '1.3', marginBottom: '3px' }}>{name}</div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '.08em' }}>{cat}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 28px 80px' }}>
        <div style={{ background: 'linear-gradient(135deg, #1E1B4B, #0C4A6E)', borderRadius: '20px', padding: '56px 48px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '16px', lineHeight: '1.7', maxWidth: '52ch', margin: '0 auto 36px' }}>
      ...
      <footer style={{ borderTop: '1px solid var(--border-subtle)', background: 'var(--bg-surface)', padding: '32px 28px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: '700', fontSize: '15px', color: 'var(--text-primary)' }}>Vinay Gangidi</span>
          </div>
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', color: 'var(--text-secondary)' }}>Enterprise AI & Automation Architect · Morrisville, NC</span>
